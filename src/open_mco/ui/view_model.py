...
def continuous_planned_state(
    progress: float,
    aircraft: AircraftDefinition,
    flight_plan: FlightPlanEstimate,
) -> dict[str, float | str]:
    """Interpolate the source-backed phase envelope over the complete route.

    Route progress is mapped through the calculated phase distances. The climb uses every
    aircraft profile point, while descent and landing use the explicit phase endpoints from the
...
def observed_flight_state(route: Route, progress: float) -> dict[str, object]:
    ...
    if current.barometric_altitude_m is not None and following.barometric_altitude_m is not None:
        altitude_m = (
            current.barometric_altitude_m
            + (following.barometric_altitude_m - current.barometric_altitude_m) * local
        )
